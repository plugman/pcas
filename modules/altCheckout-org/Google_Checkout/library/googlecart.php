  ...
  */
  ...
  class GoogleCart {
    ...
    function GetXML() {
		...
		global $config, $totalDiscount; // hack for discounts
	  ...
	  // start hack for discounts
	  if($totalDiscount>0) {
		   $xml_data->Push('item');
			$xml_data->Element('item-name', 'Discount');
			$xml_data->Element('item-description', '');
			$xml_data->Element('unit-price', -$totalDiscount,
				array('currency'=> $config['defaultCurrency']));
			$xml_data->Element('quantity', 1);
			$xml_data->Pop('item');
	  }
	  // end hack for discounts
      $xml_data->Pop('items');
												...
												$xml_data->Push('alternate-tax-rule');
													...
													if($curr_rule->country_area == "US") {
														  if($current!=="FALSE") {
															...
															$xml_data->Element('country-code',$curr_rule->country_area);
    ...
    function CheckoutButtonCode($size="large", $style="white", 
      ...
      if ($variant == "text") {
        $data =
          "<form method=\"POST\" action=\"". $this->checkout_url . "\" style=\"margin:0px; padding: 0px; text-align: right;\">
