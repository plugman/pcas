...
$view_cat = new XTemplate ("content".CC_DS."viewUnlocks.tpl");
...
////////////////////////
if (!isset($_GET['catId'])) {
	$query	= "SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category WHERE cat_father_id = 0 AND hide = '0' AND type = '0' ORDER BY priority,cat_name ASC";
	...
	$query	= "SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category WHERE cat_id = ".$db->mySQLSafe($_GET['catId']). " AND hide = '0' AND type = '0' ORDER BY priority,cat_name ASC";
}
...
$docresult = $db->select("SELECT doc_content,doc_metatitle,doc_metadesc,doc_metakeywords FROM ".$glob['dbprefix']."ImeiUnlock_docs WHERE doc_id = ".$db->mySQLSafe(20));
$foreignDocs = $db->select("SELECT doc_content,doc_master_id as doc_id, doc_name FROM ".$glob['dbprefix']."ImeiUnlock_docs_lang WHERE doc_lang = '" . LANG_FOLDER . "' AND doc_master_id = ".$db->mySQLSafe(20));
if($foreignDocs)
$docresult[0]['doc_content'] = $foreignDocs[0]['doc_content'];

$view_cat->assign("TXT_MAIN_TEXT", $docresult[0]['doc_content']);
