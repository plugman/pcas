...
// Dangerous var fixed $case_cart->assign("VAL_BACK_TO", $_GET['_a']);
...
$case_cart = new XTemplate ("content".CC_DS."casecart.tpl");
$case_cart->assign("LANG_CART",$lang['cart']['cart']);
$case_cart->assign("LANG_CHECKOUT", $lang['cart']['checkout']);
$case_cart->assign("LANG_PAYMENT", $lang['cart']['payment']);
$case_cart->assign("LANG_COMPLETE", $lang['cart']['complete']);
$case_cart->assign("LANG_ADD_PRODCODE",$lang['cart']['add_more']);
$case_cart->assign("LANG_ADD", $lang['cart']['add']);
$case_cart->assign("LANG_QTY",$lang['cart']['qty']);
$case_cart->assign("LANG_PRODUCT",$lang['cart']['product']);
$case_cart->assign("LANG_CODE",$lang['cart']['code']);
$case_cart->assign("LANG_STOCK",$lang['cart']['stock']);
$case_cart->assign("LANG_PRICE",$lang['cart']['price']);
$case_cart->assign("LANG_LINE_PRICE",$lang['cart']['line_price']);
$case_cart->assign("LANG_DELETE",$lang['cart']['delete']);
$case_cart->assign("LANG_REMOVE_ITEM",$lang['cart']['remove']);

if ($_GET['_a'] == 'cart') {

	$case_cart->assign("CONT_VAL","index.php?_g=co&_a=step1");
	$case_cart->assign("LANG_CHECKOUT_BTN",$lang['cart']['checkout_btn']);
	$case_cart->assign("LANG_case_cart",$lang['cart']['case_cart']);
	$case_cart->assign("CLASS_CART","class='txtcartProgressCurrent'");
	$case_cart->assign("CLASS_STEP2","");
...
#	if (empty($basket)) httpredir('?_g=co&_a=cart');
	...
	// Place Order Link
	$case_cart->assign("CONT_VAL", "index.php?_g=co&_a=step3");
	$case_cart->assign("CONT_VAL", ($_GET['editDel'] == 1) ? "javascript:submitDoc('cart');" : "index.php?_g=co&_a=step3");
	
	$case_cart->assign("CLASS_CART","");
	$case_cart->assign("CLASS_STEP2","class='txtcartProgressCurrent'");
	$case_cart->assign("LANG_case_cart",$lang['cart']['place_order_title']);

	$case_cart->assign("LANG_INVOICE_ADDRESS",$lang['cart']['invoice_address']);
	$case_cart->assign("LANG_DELIVERY_ADDRESS",$lang['cart']['delivery_address']);
	
	$case_cart->assign("TXT_TITLE",$lang['cart']['title']);
	$case_cart->assign("LANG_TITLE_DESC",$lang['reg']['title_desc']);

	$case_cart->assign("TXT_FIRST_NAME",$lang['cart']['first_name']);
	$case_cart->assign("TXT_LAST_NAME",$lang['cart']['last_name']);
	$case_cart->assign("TXT_COMPANY_NAME",$lang['cart']['company_name']);
	$case_cart->assign("TXT_ADD_1",$lang['cart']['address2']);
	$case_cart->assign("TXT_ADD_2","");
	$case_cart->assign("TXT_TOWN",$lang['cart']['town']);
	$case_cart->assign("TXT_COUNTY",$lang['cart']['county']);
	$case_cart->assign("TXT_POSTCODE",$lang['cart']['postcode']);
	$case_cart->assign("TXT_COUNTRY",$lang['cart']['country']);
	...
	if (isset($_SESSION['token']) && isset($_SESSION['payer_id'])) {
		...
		$case_cart->assign("LANG_CHECKOUT_BTN",$lang['gateway']['continue']);
	} else {
		$case_cart->assign("LANG_CHECKOUT_BTN",$lang['cart']['place_order']);
	}
	...
	// stick in delivery details
	$case_cart->assign("VAL_DEL_TITLE", stripslashes(html_entity_decode($basket['delInf']['title'])));
	$case_cart->assign("VAL_DEL_FIRST_NAME",stripslashes(html_entity_decode($basket['delInf']['firstName'])));
	$case_cart->assign("VAL_DEL_LAST_NAME",stripslashes(html_entity_decode($basket['delInf']['lastName'])));
	$case_cart->assign("VAL_DEL_COMPANY_NAME",stripslashes(html_entity_decode($basket['delInf']['companyName'])));
	$case_cart->assign("VAL_DEL_ADD_1",stripslashes(html_entity_decode($basket['delInf']['add_1'])));
	$case_cart->assign("VAL_DEL_ADD_2",stripslashes(html_entity_decode($basket['delInf']['add_2'])));
	$case_cart->assign("VAL_DEL_TOWN",stripslashes(html_entity_decode($basket['delInf']['town'])));
	$case_cart->assign("VAL_DEL_COUNTY",stripslashes(html_entity_decode($basket['delInf']['county'])));
	$case_cart->assign("VAL_DEL_POSTCODE",stripslashes(html_entity_decode($basket['delInf']['postcode'])));
	$case_cart->assign("VAL_DEL_COUNTRY",getCountryFormat($basket['delInf']['country'],"id","printable_name"));
	
	// stick in invoice details
	$case_cart->assign("VAL_TITLE",stripslashes($cc_session->ccUserData['title']));
	$case_cart->assign("VAL_FIRST_NAME",stripslashes($cc_session->ccUserData['firstName']));
	$case_cart->assign("VAL_LAST_NAME",stripslashes($cc_session->ccUserData['lastName']));
	$case_cart->assign("VAL_COMPANY_NAME",stripslashes($cc_session->ccUserData['companyName']));
	$case_cart->assign("VAL_ADD_1",stripslashes($cc_session->ccUserData['add_1']));
	$case_cart->assign("VAL_ADD_2",stripslashes($cc_session->ccUserData['add_2']));
	$case_cart->assign("VAL_TOWN",stripslashes($cc_session->ccUserData['town']));
	$case_cart->assign("VAL_COUNTY",stripslashes($cc_session->ccUserData['county']));
	$case_cart->assign("VAL_POSTCODE",stripslashes($cc_session->ccUserData['postcode']));
	$case_cart->assign("VAL_COUNTRY",getCountryFormat($cc_session->ccUserData['country'],"id","printable_name"));
	
	$case_cart->assign("LANG_CHANGE_INV_ADD",$lang['cart']['edit_invoice_address']);
	$case_cart->assign("VAL_BACK_TO", $current_a);
	...
	if (isset($_GET['editDel']) && $_GET['editDel'] == true && !$config['shipAddressLock']) {
		...
		if (is_array($counties)){
			$case_cart->assign("VAL_COUNTY_SEL_STYLE", "style='display:block;'");
			$case_cart->assign("VAL_COUNTY_TXT_STYLE", "style='display:none;'");
			$case_cart->assign("VAL_COUNTY_WHICH_FIELD", "S");
		} else {
			$case_cart->assign("VAL_COUNTY_SEL_STYLE", "style='display:none;'");
			$case_cart->assign("VAL_COUNTY_TXT_STYLE", "style='display:block;'");
			$case_cart->assign("VAL_COUNTY_WHICH_FIELD", 'T');
		}
		$case_cart->assign('JS_COUNTY_OPTIONS', '<script type="text/javascript">'.$jsScript.'</script>');
	
		for ($i=0; $i<count($counties); $i++) {
			if ($counties[$i]['name'] == $basket['delInf']['county']){
				$case_cart->assign('COUNTY_SELECTED', 'selected="selected"');
			} else {
				$case_cart->assign('COUNTY_SELECTED', '');
			}
	
			$countyName = $counties[$i]['name'];
			if (strlen($countyName)>20) $countyName = substr($countyName ,0, 20).'&hellip;';
			
			$case_cart->assign('VAL_DEL_COUNTY_NAME', $countyName);
			$case_cart->parse('case_cart.cart_true.edit_delivery.county_opts');
		}
		...
		for ($i=0; $i<count($countries); $i++) {
			if ($countries[$i]['id'] == $basket['delInf']['country']) {
				$case_cart->assign('COUNTRY_SELECTED', 'selected="selected"');
			} else {
				$case_cart->assign('COUNTRY_SELECTED', '');
			}
			$case_cart->assign("VAL_DEL_COUNTRY_ID",$countries[$i]['id']);
			...
			$case_cart->assign('VAL_DEL_COUNTRY_NAME', $countryName);
			$case_cart->parse('case_cart.cart_true.edit_delivery.country_opts');
		}
		$case_cart->parse('case_cart.cart_true.edit_delivery');
	} else {
		if (!$config['shipAddressLock']) {
			$case_cart->assign('LANG_CHANGE_DEL_ADD', $lang['cart']['edit_delivery_address']);
			$case_cart->parse('case_cart.cart_true.fixed_delivery.edit_btn');
		}
		$case_cart->parse("case_cart.cart_true.fixed_delivery");
	}
}
...
if ($basket['conts']) {
	...
	foreach($basket['conts'] as $key => $value){
		...
		if ($basket['conts'][$key]['custom'] == 1) {
			...
			$case_cart->assign("VAL_OPT_NAME",$lang['cart']['gift_cert_recip_name']);
			$case_cart->assign("VAL_OPT_VALUE",$basket['conts'][$key]['gcInfo']['recipName']);
				
			$plainOpts .= $lang['cart']['gift_cert_recip_name']." - ".$basket['conts'][$key]['gcInfo']['recipName']."\r\n";
			$case_cart->parse("case_cart.cart_true.repeat_cart_contents.options");
			
			$case_cart->assign("VAL_OPT_NAME",$lang['cart']['gift_cert_recip_email']);
			$case_cart->assign("VAL_OPT_VALUE",$basket['conts'][$key]['gcInfo']['recipEmail']);
				
			$plainOpts .= $lang['cart']['gift_cert_recip_email']." - ".$basket['conts'][$key]['gcInfo']['recipEmail']."\r\n";
			$case_cart->parse("case_cart.cart_true.repeat_cart_contents.options");
			
			$case_cart->assign("VAL_OPT_NAME",$lang['cart']['gift_cert_recip_message']);
			
			$gcMessage = $basket['conts'][$key]['gcInfo']['message'];
		
			if (strlen($gcMessage) > 30) $gcMessage = substr($gcMessage, 0, 30).'&hellip;';
			
			$case_cart->assign("VAL_OPT_VALUE", $gcMessage);
				
			$plainOpts .= $lang['cart']['gift_cert_recip_message']." - ".$basket['conts'][$key]['gcInfo']['message']."\r\n";
			$case_cart->parse("case_cart.cart_true.repeat_cart_contents.options");
			
			$case_cart->assign("VAL_OPT_NAME",$lang['cart']['gift_cert_delivery']);
			$case_cart->assign("VAL_OPT_VALUE",$lang['cart']['delivery_method_'.$basket['conts'][$key]['gcInfo']['delivery']]);
			...
			$product[0]['name']			= $basket['conts'][$key]['caseInfo']['name'];
			 $product[0]['image']		= $GLOBALS['rootRel']."uploads/userdata/".$cc_session->ccUserData['sessId']."/".$basket['conts'][$key]['caseInfo']['image'];
			$product[0]['cat_id']		= 0;
			$product[0]['sale_price']	= 0;
			$product[0]['stock_level']	= 0;
			$product[0]['useStockLevel']= 0;
			...
			// START PSEUDO PRODUCT OPTS
			$case_cart->assign("VAL_OPT_NAME","Design Name");
			$case_cart->assign("VAL_OPT_VALUE",$basket['conts'][$key]['caseInfo']['designname']);
				
			
			$case_cart->parse("cartpopup.cart_true.repeat_cart_contents.options");
		...
		$case_cart->assign("TD_CART_CLASS",cellColor($i, 'tdcartEven', 'tdcartOdd'));
		$case_cart->assign("VAL_PRODUCT_ID",$productId);
		$case_cart->assign("VAL_CURRENT_STEP",$current_a);
		$case_cart->assign("VAL_PRODUCT_KEY", md5($key));
		
		if ($product[0]['giftCert'] === true) {
			$case_cart->assign('VAL_IMG_SRC', imgPath($product[0]['image'], false, ''));
		} elseif ($product[0]['case'] === true) {
			$case_cart->assign('VAL_IMG_SRC', imgPath($product[0]['image'], false, ''));
		}else {
			if (file_exists(imgPath($product[0]['image'], true, 'root')) && !empty($product[0]['image'])) {
				$case_cart->assign('VAL_IMG_SRC', imgPath($product[0]["image"], true, 'rel'));
			} else {
				$case_cart->assign('VAL_IMG_SRC', 'skins/'. SKIN_FOLDER . '/styleImages/thumb_nophoto.gif');
			}
		}
		...
		$case_cart->assign("VAL_PRODUCT_NAME", validHTML($product[0]["name"]));
		$case_cart->assign("VAL_PRODUCT_CODE", $product[0]["productCode"]);  
		...
		if (!empty($optionKeys)) {
			...
			foreach ($options as $value) {
				...
				if (isset($option_value) && isset($option_name)) {
					## Assign values
					$case_cart->assign('VAL_OPT_NAME', validHTML($option_name));
					$case_cart->assign('VAL_OPT_VALUE', htmlentities(strip_tags($option_value), ENT_QUOTES, 'UTF-8'));
		...
		if ($product[0]['useStockLevel'] && $config['stockLevel']){
			$case_cart->assign('VAL_INSTOCK', $product[0]['stock_level']);
		} else {
			$case_cart->assign('VAL_INSTOCK', '&infin;');
		}
		
		if (($config['outofstockPurchase']) && ($product[0]["stock_level"]<$cart->cartArray['conts'][$key]["quantity"]) && ($product[0]['useStockLevel'])) {
			$case_cart->assign("VAL_STOCK_WARN",$lang['cart']['stock_warn']);
			
			$quantity = $cart->cartArray['conts'][$key]["quantity"];
			$case_cart->parse("case_cart.repeat_cart_contents.stock_warn");
		
		} else if ((!$config['outofstockPurchase']) && ($product[0]["stock_level"]<$cart->cartArray['conts'][$key]["quantity"]) && ($product[0]['useStockLevel'])) {

			$case_cart->assign("VAL_STOCK_WARN",$lang['cart']['amount_capped']." ".$product[0]["stock_level"].".");
			$quantity = $product[0]["stock_level"];
			$basket = $cart->update($key, $quantity);
			$case_cart->parse("case_cart.cart_true.repeat_cart_contents.stock_warn");
		} else {
			$quantity = $cart->cartArray['conts'][$key]["quantity"];
		}
		$case_cart->assign("VAL_QUANTITY", $quantity);
		...
		if ($basket['conts'][$key]['custom'] == "1") {
			...
			$case_cart->parse("case_cart.cart_true.repeat_cart_contents.quanDisabled");
		} else {
			$case_cart->parse("case_cart.cart_true.repeat_cart_contents.quanEnabled");
		}
		
		$case_cart->assign("VAL_IND_PRICE", priceFormat($price, true));
		$case_cart->assign("VAL_LINE_PRICE", priceFormat($linePrice, true));
		...
		$subTotal += $linePrice;
		$case_cart->parse("case_cart.cart_true.repeat_cart_contents");
	}
	...
	*/
	...
	$case_cart->assign("LANG_SHIPPING", $lang['cart']['shipping']);
	
	$case_cart->assign("VAL_SHIPPING", $shippingPrice);
	...
	if ($config_tax_mod['status']){
		...
		if (count($taxes)==0){
			$case_cart->assign("LANG_TAX",$lang['cart']['tax']);
			$case_cart->assign("VAL_TAX",priceFormat(0,TRUE));
		} else {
			// any additional taxes
			for ($i=1; $i<count($taxes); $i++) {
				$case_cart->assign("LANG_TAX",$taxes[$i]['display']);
				$case_cart->assign("VAL_TAX",priceFormat($taxes[$i]['amount'],TRUE));
				$case_cart->parse("case_cart.cart_true.repeat_more_taxes");
			}

			// first tax
			$case_cart->assign("LANG_TAX",$taxes[0]['display']);
			$case_cart->assign("VAL_TAX",priceFormat($taxes[0]['amount'],TRUE));
		}
		...
		$case_cart->assign("VAL_TAX_REG",$reg_string);
		// end: Flexible Taxes
	} else {
		$case_cart->assign("LANG_TAX",$lang['cart']['tax']);
		$case_cart->assign("VAL_TAX",priceFormat($tax,TRUE));
	}
	...
	$case_cart->assign("LANG_DISCOUNT",$lang['cart']['discount']);
	// set discount to return value if null
	if(empty($basket['discount']) || !isset($basket['discount'])) { 
		$basket['discount'] = 0; 
	} 
	$case_cart->assign("VAL_DISCOUNT", priceFormat($basket['discount'], true));

	$case_cart->assign("LANG_SUBTOTAL", $lang['cart']['subtotal']);
	$case_cart->assign("VAL_SUBTOTAL", priceFormat($subTotal, true));
	
	if($totalWeight>0){
		$case_cart->assign("LANG_BASKET_WEIGHT",$lang['cart']['basket_weight']);
		$case_cart->assign("VAL_BASKET_WEIGHT",$totalWeight.$config['weightUnit']);
	}
	...
	$case_cart->assign("LANG_CART_TOTAL",$lang['cart']['cart_total']);
	$case_cart->assign("VAL_CART_TOTAL",priceFormat($grandTotal, true));
	
	if (isset($basket['codeResult'])) {
		if (!$basket['codeResult']) {
			# add remove link
			$case_cart->assign("LANG_CODE_REMOVE",$lang['cart']['coupon_remove']);
			$case_cart->assign("VAL_OLD_CODE",base64_encode($basket['code']));
			$case_cart->assign("VAL_CURRENT_PAGE",currentPage());
			$case_cart->parse("case_cart.cart_true.coupon_code_result.remove");
		}
		$case_cart->assign("LANG_CODE_RESULT",$lang['cart']['coupon_result_'.$basket['codeResult']]);
		$case_cart->parse("case_cart.cart_true.coupon_code_result");
		
	}

	if (!isset($basket['codeResult']) || $basket['codeResult']>0) {
		$case_cart->assign("LANG_GOT_CODE", $lang['cart']['got_code']);
		$case_cart->assign("LANG_ENTER_CODE", $lang['cart']['enter_code']);
		$case_cart->parse("case_cart.cart_true.enter_coupon_code");
	...
	$case_cart->assign("LANG_UPDATE_CART_DESC",$lang['cart']['if_changed_quan']);
	
	$case_cart->assign("LANG_UPDATE_CART",$lang['cart']['update_cart']);
	...
	if($grandTotal>0 && !isset($_SESSION['token']) && !isset($_SESSION['payer_id']) && $altCheckout) {
		...
		if ($customWarn) {
			$case_cart->assign("LANG_CUSTOM_WARN",$lang['cart']['custom_warn']);
			$case_cart->parse("case_cart.cart_true.alt_checkout.custom_warn");	
		}
		$case_cart->assign("LANG_ALTERNATIVE_CHECKOUT",$lang['cart']['alternative']);
		...
		if (!isset($customWarn) || $customWarn != true) {
			...
			for ($i=0;$i<count($altCheckout);$i++) {
				...
				if($altCheckoutConf['mode']!=="USDPO") {
					...
					$case_cart->assign("IMG_CHECKOUT_ALT",$buttonCode);
					$case_cart->parse("case_cart.cart_true.alt_checkout.loop_button");
		...
		$case_cart->parse("case_cart.cart_true.alt_checkout");	
	}
	$case_cart->assign("VAL_FORM_ACTION", currentPage());
	$case_cart->parse("case_cart.cart_true");
} else {
	...
	$case_cart->assign("LANG_CART_EMPTY", $lang['cart']['cart_empty']);
	$case_cart->parse("case_cart.cart_false");

} 

$case_cart->parse("case_cart");
$page_content = $case_cart->text("case_cart");
?>