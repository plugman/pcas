...
$onclick = "jQuery('#cart-box a.close').click();";
	$box_content->assign("CONT_VAL","index.php?_a=reg&amp;redir=step3");
...
#	if (empty($basket)) httpredir('?_g=co&_a=cart');
	...
	// Place Order Link
	$box_content->assign("CONT_VAL", "index.php?_g=co&_a=step3");
	$box_content->assign("CONT_VAL", ($_GET['editDel'] == 1) ? "javascript:submitDoc('cart');" : "index.php?_g=co&_a=login");
...
if ($basket['conts']) {
	...
	foreach($basket['conts'] as $key => $value){
		...
		$box_content->assign("LANG_IMEI","IMEI #:");	
		$box_content->assign("VAL_IMEI",$imei);
		}
