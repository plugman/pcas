...
echo $reciveddata[1];
	...
	if($allimages){
		...
		if($allimages[0]['cover_photo']){
			$icnSrc = imgPath($allimages[0]['cover_photo'],'',$path="profimg", $_POST['userfolder']);
			$icnSrcroot = imgPath($allimages[0]['cover_photo'],'',$path="profimgroot", $_POST['userfolder']);
			// Tiny Image				
				...
				$tiny->size_auto(970);	// [OPTIONAL] set the biggest width or height for thumbnail				
