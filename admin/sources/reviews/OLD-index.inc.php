...
if($_GET['edit']>0) {
	...
	$query = "SELECT I.name as `prodName`, I.productId, R.* FROM `".$glob['dbprefix']."ImeiUnlock_reviews` AS R INNER JOIN `".$glob['dbprefix']."ImeiUnlock_inventory` AS I WHERE R.productId = I.productId ".$where." ORDER BY ". $orderBy;
}
...
<span style="float: right;">
	...
	?>
	<a <?php if(permission("reviews","edit")==TRUE) { ?>href="<?php echo $currentPage; ?>&amp;approved=0&amp;id=<?php echo $results[$i]['id']; ?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['sure_q'])); ?>')"<?php } else { echo $link401; } ?> class="txtRed"><?php echo $lang['admin']['reviews_unpublish'];?></a>
	<?php
	} else {
	?> 
	<a <?php if(permission("reviews","edit")==TRUE) { ?>href="<?php echo $currentPage; ?>&amp;approved=1&amp;id=<?php echo $results[$i]['id']; ?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['sure_q'])); ?>')"<?php } else { echo $link401; } ?> class="txtGreen"><?php echo $lang['admin']['reviews_publish'];?></a>
	<?php
	} 
	?>
	/ 
	...
	/ 
	<a <?php if(permission("reviews","delete")==TRUE) { ?>href="<?php echo $glob['adminFile']; ?>?_g=reviews/index&amp;delete=<?php echo $results[$i]['id'];?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['delete_q'])); ?>')"<?php } else { echo $link401; } ?> class="txtLink"><?php echo $lang['admin_common']['delete'];?></a>
	</span>
