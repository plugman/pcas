...
if (isset($_GET['hide'])) {	
	...
	$record["parent"] = $db->mySQLSafe($_POST['device_id']);	
...
if (!isset($_GET['mode'])) {
	...
	if (isset($_GET['edit']) && $_GET['edit'] > 0) {
		...
		$query = sprintf("SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_case_models WHERE parent = %s", $db->mySQLSafe($_GET['device'])); 
	}
...
if (!isset($_GET['mode'])) {
	...
	$url = (permission("case","write") == true) ? 'href="?_g=case/casemodels&amp;device='.$_GET['device'].'&amp;mode=new" class="txtLink"' : $link401;
...
<?php
	...
	<?php
		...
		$url = (permission("case","edit")) ? 'href="?_g=case/casemodels&amp;device='.$_GET['device'].'&amp;edit='.$results[$i]['id'].'" class="txtLink"' : $link401;
	...
	<?php
		...
		if (permission("case","delete")) {
				...
				$url = 'href="?_g=case/casemodels&amp;device='.$_GET['device'].'&amp;delete='.$results[$i]['id'].'" onclick=" return confirm(\''.str_replace("\n", '\n', $lang['admin_common']['delete_q']).'\');" class="txtLink"';
  	...
  	<?php
		...
		switch($results[$i]['hide']) {
			...
			case 0:
				...
				$url	= (permission("case","edit")) ? 'href="?_g=case/casemodels&amp;device='.$_GET['device'].'&amp;hide=1&amp;id='.$results[$i]['id'].'" class="txtLink"' : $link401;
			...
			case 1:
				...
				$url	= (permission("case","edit")) ? 'href="?_g=case/casemodels&amp;device='.$_GET['device'].'&amp;hide=0&amp;id='.$results[$i]['id'].'" class="txtLink"' : $link401;
...
<form action="?_g=case/casemodels&amp;device=<?php echo $_GET['device']; ?>" method="post" enctype="multipart/form-data" name="form1">
...
<table border="0" cellspacing="1" cellpadding="3" class="mainTable" width="100%">
  ...
  <tr>
    ...
    <td width="20%" class="tdText"><strong>Case Type Name</strong></td>
...
<tr>
  ...
  <tr>
    ...
    <td width="17%" class="tdText" style="font-weight:bold;">Output Design Width:</td>
  ...
  <tr>
    ...
    <td width="17%" class="tdText" style="font-weight:bold;">Output Design height:</td>
