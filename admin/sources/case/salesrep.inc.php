...
if (isset($_GET['hide'])) {
	...
	$update			= $db->update($glob['dbprefix']."ImeiUnlock_sales_rep", $record, $where);
	...
	// delete index
	$delete = $db->delete($glob['dbprefix']."ImeiUnlock_sales_rep", $where);
	...
	if (is_numeric($_POST['id'])) {
		...
		$insert = $db->insert($glob['dbprefix']."ImeiUnlock_sales_rep", $record);
...
if (!isset($_GET['mode'])) {
	...
	if (isset($_GET['edit']) && $_GET['edit'] > 0) {
		$query = sprintf("SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_sales_rep WHERE id = %s", $db->mySQLSafe($_GET['edit'])); 
	} else {
		//$whereClause = (is_numeric($_GET['parent'])) ? sprintf("cat_father_id = '%d'", $_GET['parent']) : 'cat_father_id = 0';
		$query = "SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_sales_rep";
	}
...
<?php
if (!isset($_GET['mode'])) {
	$url = (permission("products","write") == true) ? 'href="?_g=repair/salesrep&amp;mode=new&amp;parent='.$_GET['parent'].'" class="txtLink"' : $link401;
...
if ($results == true) {
	...
	foreach ($results as $i => $result) {
		...
		$sql		= sprintf("SELECT id FROM %sImeiUnlock_sales_rep WHERE id = '%d'", $glob['dbprefix'], $results[$i]['id']);
  ...
  <div class="action2">
  	<div class="right2"><?php
		switch($results[$i]['hide']) {
			...
			case 0:
				$url	= (permission("products","edit")) ? 'href="?_g=repair/salesrep&amp;hide=1&amp;id='.$results[$i]['id'].'" class="txtLink"' : $link401;
				$title	= $lang['admin_common']['show'];
				break;
			case 1:
				$url	= (permission("products","edit")) ? 'href="?_g=repair/salesrep&amp;hide=0&amp;id='.$results[$i]['id'].'" class="txtLink"' : $link401;
	...
	<div class="right2"><?php
		if (permission("products","delete") && $results[$i]['id'] != 0) {
			if ($results[$i]['noProducts'] <= 0) {
				$url = 'href="?_g=repair/salesrep&amp;delete='.$results[$i]['id'].'" onclick=" return confirm(\''.str_replace("\n", '\n', $lang['admin_common']['delete_q']).'\');" class="txtLink"';
			} else {
	...
	?></div>
	<div class="right2"><?php
		$url = (permission("products","edit")) ? 'href="?_g=repair/salesrep&amp;edit='.$results[$i]['id'].'" class="txtLink"' : $link401;
	...
	<?php if ($i >= 1) { ?>
	  <a href="?_g=repair/salesrep&amp;move=<?php echo $results[$i]['id']; ?>&amp;to=<?php echo $i-1; ?>&amp;parent=<?php echo $results[$i]['cat_father_id']; ?>&amp;dir=up">
	  <img src="<?php echo $glob['rootRel']; ?>images/admin/arrow_up.gif" border="0" /></a>
	<?php  } ?>
	<?php if ($i < $count) { ?>
	  <a href="?_g=repair/salesrep&amp;move=<?php echo $results[$i]['id']; ?>&amp;to=<?php echo $i+1; ?>&amp;parent=<?php echo $results[$i]['cat_father_id']; ?>&amp;dir=down">
...
<form action="?_g=repair/salesrep" method="post" enctype="multipart/form-data" name="form1">
