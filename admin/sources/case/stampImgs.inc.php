...
if (isset($_POST['saveOrder']) && !empty($_POST['priority'])) {
	...
	foreach ($_POST['priority'] as $index => $id) {
		$sql = sprintf("UPDATE %sImeiUnlock_stamp_img SET priority = '%d' WHERE id = '%d' LIMIT 1;", $glob['dbprefix'], $index+1, $id);
		$db->misc($sql);
	}
}
if(isset($_GET['mode'])&& $_GET['mode']=="update"){
	$tempimages = $db->select("SELECT * from tbl_tmpimg_idx where productId = ". $db->mySQLSafe($_GET['stampId']));
	...
	if(!empty($tempimages)){
		...
		for($i=0;$i<$counttemp;$i++){
			...
			if(file_exists($tempfile)){ 
				...
				$insertimage			= $db->insert("ImeiUnlock_stamp_img", $imgindx);			
				unset($imgindx);
...
{
	$idx = $db->select(sprintf("SELECT * FROM %sImeiUnlock_stamp_img WHERE id = %d limit 0,1;", $glob['dbprefix'], $_GET['remove']));
	...
	$delete = sprintf("DELETE FROM %sImeiUnlock_stamp_img WHERE id = %s", $glob['dbprefix'], $_GET['remove']);
	$db->misc($delete);
	unset($query);
...
/*$query		= sprintf("SELECT image FROM %sCubeCart_inventory WHERE productId = %d LIMIT 1;", $glob['dbprefix'], $_GET['stampId']);
$product	= $db->select($query);*/


## Get the main image from the inventory table
$query		= sprintf("SELECT * FROM %sImeiUnlock_stamp_img WHERE productId = %d ORDER BY id ASC;", $glob['dbprefix'], $_GET['stampId']);
...
<body>
<div style="float:right;"><a <?php if(permission("products","write")==TRUE){ ?>href="<?php echo $glob['adminFile']; ?>?_g=case/stampImgs&amp;stampId=<?=$_REQUEST['stampId']?>&amp;mode=new" class="txtLink" <?php } else { echo $link401; } ?>><img src="<?php echo $glob['adminFolder']; ?>/images/buttons/new.gif" alt="" hspace="4" border="0" title="" /><?php echo $lang['admin_common']['add_new'];?></a></div>
...
<table border="0" width="100%" cellspacing="1" cellpadding="3" class="mainTable">
  ...
  <tr>
    <td>&nbsp;</td>
    <td  class="copyText"><a class="submit" href="<?php echo $glob['adminFile']; ?>?_g=case/stampImgs&amp;stampId=<?=$_REQUEST['stampId']?>&mode=update" style="text-decoration:none; padding:2px 2px 2px 2px"> Save Record </a></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<div id="columnHeaders" class="tdTitle">
  ...
  <?php 
	  	if(!empty($imageidx))
		{
			...
			//echo $thumbImg;
	  ?>
 <div id="product_<?php echo $imageidx[$i]['id']; ?>" class="imgrow <?php echo $cellColor; ?> tdText" style="position:relative">
 <input type="hidden" name="priority[]" value="<?php echo $imageidx[$i]['id']; ?>" />
    ...
    right: 60px;
    top: 50px;"><a href="<?php echo $glob['adminFile']; ?>?_g=case/stampImgs&amp;stampId=<?php echo $_GET['stampId']?>&amp;remove=<?php echo $imageidx[$i]['id']; ?>" class="txtLink"><?php echo $lang['admin_common']['remove'];?></a></span>
</div>
