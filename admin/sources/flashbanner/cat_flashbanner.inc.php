...
$req_banner_width	= 690;
$req_banner_height	= 272;

if (isset($_POST['saveOrder']) && !empty($_POST['priority'])) {
	foreach ($_POST['priority'] as $index => $img_id) {
		
		
		$sql = sprintf("UPDATE ImeiUnlock_cat_flashbanner SET priority = '%s' WHERE img_id = '%d' LIMIT 1;", $index+1, $img_id);
		$db->misc($sql);
	}
	$cache = new cache();
	$cache->clearCache();
...
if (isset($_GET['hide'])) {
	...
	$update = $db->update("ImeiUnlock_cat_flashbanner", $record, $where);
	...
	$rsVideo = $db->select("SELECT * FROM ImeiUnlock_cat_flashbanner WHERE ".$where);
	//@unlink("images/uploads/flashbanner/".$rsVideo[0]["vid_file"]);
	@unlink("images/uploads/flashbanner/".$rsVideo[0]["img_file"]);
	$delete = $db->delete("ImeiUnlock_cat_flashbanner", $where);
	...
	$record["cat_id"] 	= $db->mySQLSafe($_POST['catid']);
...
/*    if ($_FILES['filename']['name'] != "")
	    ...
	    if ($_FILES['imagename']['name'] != "")
    {
		...
		{		
			...
			else
			{
				...
				header("Location:admin.php?_g=flashbanner/cat_flashbanner&msg=$msg");
				die();
			}
		}
	...
	if (is_numeric($_POST['img_id'])) {		
		...
		$insert = $db->insert("ImeiUnlock_cat_flashbanner", $record);
...
if (!isset($_GET['mode'])) {
	...
	if (isset($_GET['edit']) && $_GET['edit'] > 0) {
		$query = sprintf("SELECT * FROM ImeiUnlock_cat_flashbanner WHERE img_id = %s", $db->mySQLSafe($_GET['edit'])); 
	} else {
		if($whereClause != "")
		$whereClause = " WHERE " . $whereClause;
		 
		$query = "SELECT * FROM ImeiUnlock_cat_flashbanner ".$whereClause." ORDER BY priority, img_title ASC";
	}
...
if (!isset($_GET['mode'])) {?><div class="maindiv" style="margin-bottom:10px"><?
	$url = 'href="?_g=flashbanner/cat_flashbanner&amp;mode=new" class="txtLink"';
...
<form method="post" id="reorderCategory" enctype="multipart/form-data">
  ...
  <div  class="headingBlackbg2"> 
	<span class="catName2" style="width:200px" >Image Title</span> 
	<span class="catid">Status</span>
	<span class="catName2"  style="width:195px">Image</span> 
    <span class="catid" style="width:148px">Category</span> 
...
?>
  <div id="flashbanner_<?php echo $results[$i]['img_id']; ?>" class="productRow <?php echo $cellColor; ?> tdText"> 
	...
	<span class="catName2" style="width:200px;">
    <?php
  ...
  ?>
	...
	<span class="catName2" style="width:195px;">
    <?php
  ...
  ?>
    ...
    <span class="catid" style="width:148px">
    <?php
	$catname=$db->select("SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category where cat_id=".$db->mySQLSafe($results[$i]['cat_id']));
	for($j=0;$j<count($catname);$j++){
			$maincatname=$db->select("SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category where cat_id=".$db->mySQLSafe($catname[$j]['cat_father_id']));
				for($k=0;$k<count($maincatname);$k++){
					if($catname[$j]['cat_name']){
						$sep1=' > ';
					}else{
						$sep1='';
					}
		echo $maincatname[$k]['cat_name'].$sep1.$catname[$j]['cat_name'];
				}
	}
  ?>
    </span>
  ...
  ?>
	...
	<?php
			switch($results[$i]['img_status']) {
				case 1:
					$url	= 'href="?_g=flashbanner/cat_flashbanner&amp;hide=0&amp;img_id='.$results[$i]['img_id'].'" class="txtLink"';
					$title	= "Hide";
					break;
				case 0:
					$url	= 'href="?_g=flashbanner/cat_flashbanner&amp;hide=1&amp;img_id='.$results[$i]['img_id'].'" class="txtLink"';
					$title	= "Show";
					break;
			}
	...
	<span class="right3">
    <?php
		$url = 'href="?_g=flashbanner/cat_flashbanner&amp;edit='.$results[$i]['img_id'].'" class="txtLink"';
		echo sprintf('<a %s>%s</a>', $url, "Edit");
	?>
    </span> 
    <span class="right3">
    <?php
		$url = 'href="?_g=flashbanner/cat_flashbanner&amp;delete='.$results[$i]['img_id'].'" class="txtLink"';
		echo sprintf('<a %s>%s</a>', $url, "Delete");
	?>
    </span> 
   <? /*<span class="right3">
    <?php
		$url = 'href="?_g=settings/map_products&amp;edit='.$results[$i]['img_id'].'" class="txtLink"';
		echo sprintf('<a %s>%s</a>', $url, "Add Related Products");
	?>
    </span> */?>
  </span>
...
?>
?>
<form action="?_g=flashbanner/cat_flashbanner" method="post" enctype="multipart/form-data" name="form1" 
...
<? } ?>
  <table border="0" cellspacing="1" cellpadding="3" class="mainTable" width="100%">
    ...
    <tr>
      ...
      <td width="82%"><input name="img_title" id="img_title" type="text" class="textbox" value="<?php if(isset($results[0]['img_title'])) echo validHTML($results[0]['img_title']); ?>" maxlength="255" /><span  style="color:#F00">&nbsp; *</span>      </td>
    </tr>
    <?php /*?><tr>
    ...
    <tr>
      ...
      <td valign="top"><div>
          <input name="imagename" class="textbox" style="width: 200px;" type="file" id="imagename" value="Browse / Upload File" />
    ...
    <tr>
      <td class="tdText" width="18%"><strong>Category:</strong></td>
      <td width="82%"><select name="catid">
      <option value="0">Select Category</option>
      <?
      	$main_cat=$db->select("SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category where cat_father_id=0 and type=1 ORDER BY cat_name ASC");
if($main_cat){

	for($i=0;$i<count($main_cat);$i++){
		?>
<option value="<? echo $main_cat[$i]['cat_id'] ?>" <?php if($results[0]['cat_id'] == $main_cat[$i]['cat_id']) {?> selected="selected" <?php } ?>><? echo $main_cat[$i]['cat_name']; ?></option>
<?
		$sub_cat=$db->select("SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category where cat_father_id=".$main_cat[$i]['cat_id']." ORDER BY cat_name ASC");
		if($sub_cat){
		for($j=0;$j<count($sub_cat);$j++){
			if($sub_cat){
				if($sub_cat[$j]['cat_name']){
				$sep=' > ';
			}else{
				$sep='';
			}
...
?>
<option value="<? echo $sub_cat[$j]['cat_id'] ?>" <?php if($results[0]['cat_id'] == $sub_cat[$k]['cat_id']) {?> selected="selected" <?php } ?>><? echo $main_cat[$i]['cat_name'].$sep.$sub_cat[$j]['cat_name']; ?></option>
<?
}
			$sub_cat1=$db->select("SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category where cat_father_id=".$sub_cat[$j]['cat_id']." ORDER BY cat_name ASC");
			for($k=0;$k<count($sub_cat1);$k++){
			if($sub_cat1){
				if($sub_cat1[$k]['cat_name']){
				$sep=' > ';
			}else{
				$sep='';
			}
?>
<option value="<? echo $sub_cat1[$k]['cat_id'] ?>" <?php if($results[0]['cat_id'] == $sub_cat1[$k]['cat_id']) {?> selected="selected" <?php } ?>><? echo $main_cat[$i]['cat_name'].$sep.$sub_cat[$j]['cat_name'].$sep.$sub_cat1[$k]['cat_name']; ?></option>
<?
			}
			}
		}
