...
if (isset($_POST['saveOrder']) && !empty($_POST['priority'])) {
	...
	buildCatList_tangible();
buildCatList_digital();
...
if (!isset($_GET['mode'])) {
	...
	if (isset($_GET['edit']) && $_GET['edit'] > 0) {
		$query = sprintf("SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category WHERE type != '2' AND  cat_id = %s", $db->mySQLSafe($_GET['edit'])); 
	...
	$whereClause = '';
		...
		if (isset($_GET['searchStr']) && !empty($_GET['searchStr'])) {
			...
			for ($i=0; $i<$noKeys;$i++) {
				...
				if (($ucSearchTerm !== "AND") && ($ucSearchTerm !== "OR")) {
					...
					$like .= "(cat_name LIKE '%".$searchArray[$i]."%') OR ";
		...
		//$whereClause .= (isset($like)) ? ' AND ' : ' ';
			...
			$whereClause .= "type = ".$_GET['category']." AND ";

		}

		$query = "SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category WHERE ".$whereClause. " type != '2' ORDER BY cat_id ASC";


	}else if($_GET['parent']){
		$whereClause = (is_numeric($_GET['parent'])) ? sprintf("cat_father_id = '%d'", $_GET['parent']) : 'cat_father_id = 0';
		$query = "SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category WHERE (cat_name != 'Imported Products' OR cat_desc != '##HIDDEN##') AND ".$whereClause." ORDER BY priority, cat_name ASC";
	}

else if($_GET['category']){
	$query = "SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category WHERE type =".$_GET['category'];
		
	}else{
			$query = "SELECT * FROM ".$glob['dbprefix']."ImeiUnlock_category WHERE type IN(0,1)";
	} 
...
?>
<form name="type" method="get" action="<?php echo $glob['adminFile']; ?>">

  <input type="hidden" name="_g" value="categories/index" />

  <p  class="copyText" style="margin-top:5px; margin-bottom:10px">

    

     <input type="text" placeholder="Containing the text" name="searchStr" class="textbox2" value="<?php if(isset($_GET['searchStr']))echo $_GET['searchStr']; ?>" />

    

    

    <input name="submit" style="margin:0 10px 0 11px" type="submit" class="submit" value="Filter" />

    <input name="Button" type="button" onclick="goToURL('parent','<?php echo $glob['adminFile']; ?>?_g=categories/index');return document.returnValue" value="Reset" class="submit" />
...
if (!isset($_GET['mode'])) {
	...
	$url = (permission("categories","write") == true) ? 'href="?_g=categories/index&amp;mode=new&amp;parent='.$_GET['parent'].'" class="txtLink"' : $link401;
...
<a href="?_g=categories/index" class="txtLink"><?php echo $lang['admin']['categories_loc_home']; ?></a><?php if (is_numeric($_GET['parent'])) echo getCatDir('', $_GET['parent'], 0, true, false, true, true); ?></p>-->
...
?>
  ...
  <tr>
	...
	$query 			= "SELECT `cat_name`, `cat_father_id`, `cat_id`, `cat_image` FROM `".$glob['dbprefix']."ImeiUnlock_category` WHERE type != '2'"; ## ORDER BY NOT NEEDED DUE TO NATCASESORT WHICH IS MORE TRUST WORTHY TO GET ALL DATA
