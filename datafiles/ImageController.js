...
function loadimagetobox(URL, src, m_parent) {
        ...
        $(image).bind("load", function () {
            ...
            var newHeight = $(image).height() * scale +4;
            var newWidth = $(image).width() * scale +4;
		...
		'z-index': 'auto'
			...
			$("#"+m_parent).html('');
			if(filterapplied != '1'){
			var active_effect = $("#filter-list-container").find(".active-filter").map(function () {
            return $(this).attr('class').split(" ")[0];
        }).get();
		appyfilter(active_effect);
		filterapplied = '1';
			}
